...
namespace testProjectBCA
{
    public partial class graphSaldoATM : Form
    {
        public graphSaldoATM()
        {
            ...
            comboBox2.DataSource = comboAreaData;
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    else
                    {
                        cmd.CommandText = "select"
                                          ...
                                          + " TransaksiAtms t join pkt p on t.kodePkt = p.kodePkt"
                                          + " where"
                                          + " tanggal between '" + dateTimePicker1.Value.ToString() + "' and '" + dateTimePicker2.Value.ToString() + "' and kanwil like '"+comboBox2.SelectedValue.ToString()+"' "
                                          + " group by"
                                          + " tanggal order by tanggal";
                    }
